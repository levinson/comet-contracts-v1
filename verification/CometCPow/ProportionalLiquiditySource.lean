...
/-!
Executable, source-shaped models for the proportional `exit_pool` arithmetic.
The shared ratio is evaluated once, then applied to every modeled token record.
Successful list execution proves the existing per-token pool-favoring theorem
for every returned token amount.
-/
